...
estet_raw <- readr::read_rds("data/IBGE_estrutura_etaria.rds")
...
make_plot <- function(sigla_input,sigla_name){
  ...
  tmp_plot[,pop_mil := fifelse(sexo == "Homens",-V1 / 1000, V1 / 1000)]
  ...
  plot_pop <- ggplot(tmp_plot) +
    geom_col(aes(x = pop_mil,y = idade_f,fill = sexo)
    ...
    geom_text(
      aes(x = pop_mil
          ,y = idade_f
          ,label = percent
          , hjust = fifelse(sexo == "Mulheres",-0.25,+1.1)
      )
      , size = 3
      #, vjust = 1
    ) + 
    ...
    scale_x_continuous(expand = c(0.25, 0.25)
                       , breaks = seq(min(tmp_plot$pop_mil)
                                      ,max(tmp_plot$pop_mil)
                                      ,length.out = 8)
                       , labels = seq(min(tmp_plot$pop_mil)
                                      ,max(tmp_plot$pop_mil)
                                      ,length.out = 8) %>% abs() %>% round(0)
    ) + 
    theme_bw() + 
  ...
  ggplot2::ggsave(filename = sprintf("figures/estrutura_etaria_sexo_rgint/%s.jpg"
                                     ,sigla_input)
                  ,plot = plot_pop
                  ,scale = 0.65
                  ,width = 20
...
# apply loop function -----
dir.create("figures/estrutura_etaria_sexo_rgint/") %>% suppressWarnings()
...
# exclude files
list_files_pop <- list.files("figures/estrutura_etaria_sexo_rgint/",full.names = TRUE)
